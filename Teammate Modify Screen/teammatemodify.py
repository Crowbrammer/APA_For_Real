...
from assets.system import App, Clock, ObjectProperty, StringProperty, Window
from assets.layout import GridLayout, BoxLayout, ScrollView
from assets.interface import Button, DropDown
from assets.screen import ScreenManager, Screen


global current_teammate


class HoldPositionsLayout(GridLayout):
	pass
...
class TeammateModifyLayout(BoxLayout):
	...
	def late_init(self, key, **kwargs):
		...
		global lsm
		self.lsm = []
		hold_positions_layout = HoldPositionsLayout(cols=1, size_hint_y=None, height='800dp')
		cute_scrollview = ScrollView(size_hint_y=None, height='400dp')
		...
		#	if positions
			hold_positions_layout.add_widget(self.lsm[i])

		hold_positions_layout.bind(minimum_height = hold_positions_layout.setter('height'))
		cute_scrollview.add_widget(hold_positions_layout)
		self.add_widget(cute_scrollview)
...
class ListTeammatesLayout(BoxLayout):
	...
	def __init__(self, **kwargs):
		...
		# Create the layout to hold the teammates, and ScrollView for the layout
		# to go into
		hold_teammates_layout = HoldPositionsLayout(cols=1, size_hint_y=None, height='800dp', spacing=10)
		cute_scrollview = ScrollView(size_hint_y=None, height=Window.height*2/3)

		for each in teammates:

			id_name = 'teammate{}'.format(each)
			btn = SwitchScreensButton(text=each, id=id_name, size_hint_y=None, height=200)

			# For each teammate, add a button with their name on it. Then add
			# this button to the teammate holder layout
			hold_teammates_layout.add_widget(btn)

		# Set the minimum_height to the layout's height
		hold_teammates_layout.bind(minimum_height=hold_teammates_layout.setter('height'))

		# Add the teammate holder layout into the ScrollView
		cute_scrollview.add_widget(hold_teammates_layout)
		self.add_widget(cute_scrollview)
